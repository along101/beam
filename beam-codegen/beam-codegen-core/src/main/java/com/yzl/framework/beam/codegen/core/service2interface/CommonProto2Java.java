...
import com.yzl.framework.beam.codegen.core.service2interface.printer.PrintServiceFile;
...
public class CommonProto2Java {
    ...
    private String pojoNameProcess(String origin) {
        ...
        return CommonUtils.upperCamelCaseProcess(cs);
    }
...
//        FileDescriptorProto fdp2 = commondProtoc.invoke(protoPath);
        for (FileDescriptorProto fdp : fileDescriptorSet.getFileList()) {
    ...
    private void serviceProcess(List<ServiceDescriptorProto> serviceDescList, String javaPackage, String outerClassName, String protoFileName) {
        for (ServiceDescriptorProto serviceDesc : serviceDescList) {
            ...
            /**
             * For the case that the name of the BeamInterface
             * is the same as the POJOs generated by protocol compiler.
             * */
            String tempBeamInterfaceName = prpcInterfaceNameProcess(serviceDesc.getName());
            if (!tempBeamInterfaceName.equals(originPojoName) && tempBeamInterfaceName.equals(outerClassName)) {
                tempBeamInterfaceName += "Service";
                logger.warn("Warn.Your BeamInterface name should not be the same as the java src generated by proto compiler , so suffix:Service is used here. ");
            }

            PrintServiceFile serviceFile = new PrintServiceFile(generatePath, javaPackage, tempBeamInterfaceName);
            try {
                serviceFile.setServiceMethods(serviceDesc.getMethodList());
                serviceFile.setPojoTypeCache(pojoTypes);
            } finally {
                serviceFile.print();
                logger.info("Finish generating the BeamInterface : " + tempBeamInterfaceName + "   <<<<<<<<<<<<<<<<<<<<<");
            }
        }
    }

    //todo what exactly are these two processes doing?
    private void messageProcess(List<DescriptorProto> messageDescList, String javaPackage, String outerClassName) {
        for (DescriptorProto messageDesc : messageDescList) {
            ...
            /**
             * We do not need to have the source code for message right now.
             * */
